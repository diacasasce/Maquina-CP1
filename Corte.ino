...
void loop() {
  ...
  */
  ...
  if (del < Max and tick < stpm and paro == 0 ) {
    tick++;
    digitalWrite(enable, HIGH);
    digitalWrite(velocidad, HIGH);  // This LOW to HIGH change is what creates the
  ...
  if (go < 1) {
    if (pas < lA) {
      if (stpm > tick) {
        ...
        //        Serial.println(pas);
        //        Serial.println("+++");
        lA = Corte(pas);          // CAMBIAR ESTA LINEA POR LA SECUENCIA QUE SE QUIERA lA="secuencia deseada"     !!! 1 !!!
...
int BloqueM500g(int paso) {
  ...
  float  velo[] = {60, 60, 60, 60, 60, 60, 0, 60, 60, 60, 60, 60, 60, 80};
